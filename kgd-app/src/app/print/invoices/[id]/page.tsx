...
export default async function InvoicePrintPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    // Use phone directly on customer if available, else fall back to primary contact
    const displayPhone = invoice.customer.phone || (contact?.phone ?? null)
    const displayContact = invoice.customer.name || contact?.name
    ...
    return (
        <>
          ...
          * { box-sizing: border-box; margin: 0; padding: 0; }
          ...
          .print-btn { position: fixed; top: 16px; right: 16px; background: #1d4ed8; color: white; border: none; padding: 8px 20px; border-radius: 6px; cursor: pointer; font-size: 14px; font-weight: 600; }
          @media print {
        ...
        `}</style>
            ...
            <div className="print-page">
                ...
                <div className="section">
                    ...
                    {invoice.customer.businessName && <div style={{ fontWeight: 700, fontSize: 14 }}>{invoice.customer.businessName}</div>}
                    <div style={{ fontWeight: invoice.customer.businessName ? 500 : 700, fontSize: invoice.customer.businessName ? 13 : 14 }}>
                        {displayContact}
                    </div>
                    ...
                    {displayPhone && <div style={{ marginTop: 4 }}>Phone: {displayPhone}</div>}
                    {invoice.customer.secondaryPhone && (
                        <div>Alt: {invoice.customer.secondaryPhone}</div>
                    )}
                </div>
